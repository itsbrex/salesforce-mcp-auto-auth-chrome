...
6. **Pin transitive SDKs that reach you through another package.** On 2026-07-28 the `mcp` Python SDK released 2.0.0, which removed the `Server.list_tools()` decorator API that `mcp-salesforce-connector` 0.1.15 was written against. Because the upstream connector didn't declare `mcp<2`, every fresh `uvx` resolution crashed at import time — all 11 of the maintainer's orgs died in the same moment, restarting Claude didn't help, and the cause was invisible without reading the per-server log. The fix at the user level was `--with "mcp<2"` in their launch args; the fix at the package level (v0.1.1) was pinning `mcp>=1.0,<2.0` in our own `pyproject.toml`. The general lesson: when your wrapper imports through a package that has loose upper bounds, you inherit that fragility. Add the pin at your layer so your users never see it. When the upstream ports to the new SDK, revisit the pin.
